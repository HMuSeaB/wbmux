...
这里早期写的是"不能，也不打算做"，那是错的——不是做不到，是当时没找到
正确的落点。现在可以了：
...
图形界面里也有同样入口（「历史搬运」一节），勾选后点一下即可。

### 为什么只复制会话文件没有用

这是最容易踩的坑。会话列表读的是数据目录下 `workbuddy.db` 的 **`sessions` 表**，
不是 `projects/` 目录。客户端确实有一个"从 `projects/` 重建索引"的函数
（`workbuddy-server` 的 `database-service.ts · rebuildSessionIndexFromHistory`），
但挖开调用点可以看到它只在**数据库损坏自愈**路径上被调用——正常启动不会跑。

所以只把 `.jsonl` 复制过去，列表里什么都不会出现。必须同时写一行索引，
`migrate` 做的就是这件事。

### 它保证不覆盖

- 目标端已有同名文件 → 跳过。用 `O_EXCL` 交给内核判定，而不是"先检查再写"，
  省掉竞态窗口。
- 目标端已有同 id 的会话行 → 跳过。
- 写索引之前先把目标端数据库**整份备份**（含 `-wal` / `-shm`），路径随结果返回。
  只拷 `.db` 是不够的：WAL 模式下最近的写入还在 `-wal` 里。
- 只给"jsonl 确实落在目标目录"的会话写索引行，不会造出点开就报错的列表项。
- 连跑两次不会产生任何新增（幂等）。

### 版本差异这一条要更正

早期这里写着"两边内核版本不一致，风险自负"。实测下来**不成立**：
国内版更新（引擎 2.147.0 > 国际版 2.137.1），而国际版用到的记录类型
（`reasoning` / `function_call_result` / `resend-fork-notice` 等）在国内版
`app.asar` 里逐个检索都命中，因此**不需要做格式转换**，直接搬运即可。

### 账号仍然不互通

搬运解决的是"看得见另一侧的历史"，不是"把两个账号并成一个"。
两套账号体系（国内受《个人信息保护法》约束、国际受 GDPR 约束）依旧独立，
登录态也不会因此互通。搬运过去的历史是**只读线索**，继续对话要回到原来的
那一侧——或者用 `wbmux run` 切到那一侧的后端。

### 记忆为什么经常被跳过

账号级记忆文件名是 `<uid>_memory.md`，所以跨账号搬运要换成目标端自己的 uid。
判定规则是三条：

1. 来源端正文为空 → 跳过（搬一个空模板过去没有意义）
2. 两端都有正文 → 跳过（记忆是模型自己维护的文本，机械拼接会产出它读不懂的东西）
3. 只有来源端有正文、目标端为空 → 填入

第 3 条是本项目唯一一处会改写已有文件的地方，边界收得很紧：只在目标端正文
为空时触发，所以不会有任何内容被丢掉。
