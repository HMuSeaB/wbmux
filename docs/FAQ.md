...
## 图形界面为什么是开浏览器，而不是原生窗口？

界面本身是一份内嵌在二进制里的网页，由 wbmux 在 `127.0.0.1` 上提供。
用 Edge / Chrome 的 `--app=` 模式打开，出来的窗口没有地址栏、没有标签页、
任务栏里是独立一项，看起来就是个本地程序。

这么做是为了保住"零第三方依赖 + 一个 runner 出全部平台"：
原生窗口库需要 cgo，会把交叉编译和可审计性一起毁掉。代价是二进制约 6.3 MB
（纯命令行版 2.5 MB），多出来的是 `net/http` 带进来的 TLS 与证书库。
详见 [DESIGN.md](DESIGN.md) 的"图形界面"一节。

## 图形界面安全吗？会不会被别人控制我的电脑？

三点约束：

- **只监听 `127.0.0.1`**。传 `--addr 0.0.0.0:…` 会被直接拒绝——这个服务
  能启动本机进程，暴露到局域网等于把机器交出去。
- **一次性令牌**。每次启动随机生成，写进访问地址，接口都要校验。
  任何网页都能向 `127.0.0.1` 发请求，没有令牌就调不动接口。
- **不联网**。界面只在本地收发，不向任何服务器上报。

关掉标签页后 30 分钟内进程会自行退出；也可以点界面上的「关闭界面」。
